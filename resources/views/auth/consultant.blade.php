            ...
            @foreach($consultants as $consultant)
                <div class="w-full max-w-sm bg-white py-4 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <div class="flex flex-col items-center">
                        ...
                        @if(isset($consultant->additionalInfo->phone))
                        ...
                        @if(isset($consultant->additionalInfo->bio))
                            <span class="text-sm text-gray-500 dark:text-gray-400 truncate">{{ $consultant->additionalInfo->bio }}</span>
                        @endif
                        <div class="flex mt-4 md:mt-6">
                            <a href="/start-chat/{{ $consultant->id }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Message</a>
                        </div>
                    </div>
                </div>
            @endforeach
